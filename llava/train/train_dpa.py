...
from llava.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token, convert_to_rgb
...
class ModelArguments:
    ...
    delta_probs_min: Optional[float] = field(default=None)
    delta_probs_max: Optional[float] = field(default=None)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if hasattr(trainer.args, "mm_tunable_parts") and (len(trainer.args.mm_tunable_parts.split(",")) == 1 and ("mm_vision_tower" in trainer.args.mm_tunable_parts)):
        # Only save Vision tower
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith("checkpoint-"):
                mm_projector_folder = os.path.join(parent_folder, "mm_vision_tower")
                os.makedirs(mm_projector_folder, exist_ok=True)
                torch.save(weight_to_save, os.path.join(mm_projector_folder, f"{current_folder}.bin"))
            else:
                torch.save(weight_to_save, os.path.join(output_dir, f"mm_vision_tower.bin"))
        return
...
def preprocess_llama3(
    ...
    max_len=2048,
    system_message: str = "Cutting Knowledge Date: December 2023\nToday Date: 26 Jul 2024\n\nYou are a helpful language and vision assistant. You are able to understand the visual content that the user provides, and assist the user with a variety of tasks using natural language.",
) -> Dict:
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        try:
            ...
            try:
                rank0_print(f"Using cairosvg to convert {image_file} to PNG.")
                import cairosvg
                png = cairosvg.svg2png(url=os.path.join(image_folder, image_file))
                image = Image.open(io.BytesIO(png))
            except Exception as cairosvg_exn:
                print(f"Failed to open image {image_file}. Exception in cairosvg:", cairosvg_exn)
                raise cairosvg_exn
        ...
        if image_aspect_ratio == "highres":
            ...
            try:
                ...
                if dist.get_rank() == 0:
                    with open('/remote-home1/cktan/reps/LLaVA-NeXT/logs/train_error.log','a') as f:
                        f.write(f'Error data: {image_file}' + '\n')
        else:
            image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
    ...
    def _get_item(self, i, preview=False) -> Dict[str, torch.Tensor]:
        ...
        if preview:
            ...
            rank0_print(f"Template:\n{tokenizer.decode(input_ids)}\n")
            targets = []
            for i in data_dict['labels'][0]:
                if i != IGNORE_INDEX:
                    targets.append(i)
                else:
                    targets.append(tokenizer.convert_tokens_to_ids('#'))
            rank0_print(f"Masked Template:\n{tokenizer.decode(targets)}")
            del input_ids, tokenizer
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    if data_args.eval_data_path is not None and data_args.eval_image_folder is not None:
        eval_data_args = copy.deepcopy(data_args)
        eval_data_args.image_folder = data_args.eval_image_folder
        eval_dataset = LazySupervisedDataset(tokenizer=tokenizer, data_path=data_args.eval_data_path, data_args=eval_data_args)
    elif data_args.eval_data_path is None and data_args.eval_image_folder is None:
        eval_dataset = None
    else:
        raise ValueError("eval_data_path and eval_image_folder should be both None or both not None")
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.delta_probs_max is not None:
        overwrite_config['delta_probs_max'] = model_args.delta_probs_max
    if model_args.delta_probs_min is not None:
        overwrite_config['delta_probs_min'] = model_args.delta_probs_min
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            from llava.model.language_model.llava_llama_dpa import LlavaLlamaDPAForCausalLM
            model = LlavaLlamaDPAForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            if "moe" in model_args.model_name_or_path.lower() or "A14B" in model_args.model_name_or_path:
                ...
                from llava.model.language_model.llava_qwen_dpa import LlavaQwenDPAForCausalLM
                model = LlavaQwenDPAForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            from llava.model.language_model.llava_gemma2_dpa import LlavaGemma2DPAForCausalLM
            model = LlavaGemma2DPAForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            rank0_print('Load gemma2')
            rank0_print("Using DPA method")
        elif "gemma" in model_args.model_name_or_path.lower():
            from llava.model.language_model.llava_gemma_dpa import LlavaGemmaDPAForCausalLM
            model = LlavaGemmaDPAForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if "mm_vision_resampler" in tunable_parts:
                rank0_print("Unfreezing mm_vision_resampler")
                for p in model.get_model().vision_resampler.parameters():
                    p.requires_grad = True
            ...
            if "mm_language_model" in tunable_parts:
                rank0_print("Unfreezing mm_language_model")
                for name, param in model.named_parameters():
