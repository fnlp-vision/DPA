...
def load_pretrained_model(model_path, model_base, model_name, vt_path=None, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="bfloat16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    '''
    This function sets delay_load=True to avoid loading empty weights in vision_tower when model_base is not None. 
    But if mm_vision_tower is in mm_tunable_parts in model config, it will load the vision_tower weights.
        
    Args:
        model_path (str): path to pretrained module,
        model_base (str): path to base model,
        vt_path (str, optional): path to vision encoder,
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    model = LlavaQwenForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=llava_cfg, attn_implementation=attn_implementation, **kwargs)
            else:
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if not vision_tower.is_loaded:
            ...
            if vt_path is not None:
                mm_vision_tower_weights = torch.load(os.path.join(vt_path, "mm_vision_tower.bin"), map_location="cpu")
                mm_vision_tower_weights = {k: v.to(torch_dtype) for k, v in mm_vision_tower_weights.items()}
                model.load_state_dict(mm_vision_tower_weights, strict=False)
                rank0_print(f"Loaded vision tower's weights from {vt_path}")
        # if device_map != "auto":
...
def load_pretrained_model_wo_low_cpu(model_path, model_base, model_name, load_8bit=False, load_4bit=False, torch_dtype="float16", device_map='auto', attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    '''
    Modify `load_pretrained_model()` to avoid confict between `init_empty_weights` and `_load_state_dict_into_model()` after calling PretrainedModel.from_pretrained().
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                lora_cfg_pretrained = LlavaMixtralConfig.from_pretrained(model_path)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaMixtralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            elif "mistral" in model_name.lower():
                from llava.model.language_model.llava_mistral import LlavaMistralConfig

                lora_cfg_pretrained = LlavaMistralConfig.from_pretrained(model_path)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaMistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            elif "gemma" in model_name.lower():
                from llava.model.language_model.llava_gemma import LlavaGemmaConfig

                lora_cfg_pretrained = LlavaGemmaConfig.from_pretrained(model_path)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaGemmaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            else:
                from llava.model.language_model.llava_llama import LlavaConfig

                lora_cfg_pretrained = LlavaConfig.from_pretrained(model_path)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            ...
            if "mixtral" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaMixtralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            elif "mistral" in model_name.lower() or "zephyr" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaMistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            elif "gemma" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaGemmaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            elif (
                "wizardlm-2" in model_name.lower()
                or "vicuna" in model_name.lower()
                or "llama" in model_name.lower()
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                llava_cfg = LlavaConfig.from_pretrained(model_path)
                model = LlavaLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=llava_cfg, **kwargs)
            elif "qwen" in model_name.lower() or "quyen" in model_name.lower():
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    from llava.model.language_model.llava_qwen_moe import LlavaQwenMoeConfig
                    if customized_config is None:
                        ...
                        llava_cfg = customized_config
                    tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                    model = LlavaQwenMoeForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=llava_cfg, attn_implementation=attn_implementation, **kwargs)
                else:
                    from llava.model.language_model.llava_qwen import LlavaQwenConfig
                    if customized_config is None:
                        llava_cfg = LlavaQwenConfig.from_pretrained(model_path)
                        ...
                        llava_cfg = customized_config
                    tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                    model = LlavaQwenForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=llava_cfg, attn_implementation=attn_implementation, **kwargs)
            else:
                raise ValueError(f"Model {model_name} not supported")
            ...
            if "mixtral" in model_name.lower():
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                model = LlavaMixtralForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)

            elif "mistral" in model_name.lower() or "zephyr" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                model = LlavaMistralForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, attn_implementation=attn_implementation, **kwargs)
            elif (
                "wizardlm-2" in model_name.lower()
                or "vicuna" in model_name.lower()
                or "llama" in model_name.lower()
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                    model = LlavaQwenMoeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                    # else:
                    #     model = LlavaQwenMoeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, attn_implementation=attn_implementation, **kwargs)
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaGemmaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            else:
                try:
                    from llava.model.language_model.llava_llama import LlavaConfig
        ...
        if model_base is not None:
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            model = AutoModelForCausalLM.from_pretrained(model_base, torch_dtype=torch.float16, low_cpu_mem_usage=False, device_map="auto")
            print(f"Loading LoRA weights from {model_path}")
            model = PeftModel.from_pretrained(model, model_path)
            print(f"Merging weights")
            model = model.merge_and_unload()
